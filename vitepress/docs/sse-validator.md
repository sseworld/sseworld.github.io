...
A library of string @sseworld/validators and sanitizers.
...
Install the `@sseworld/validator` package as:

```sh
npm i @sseworld/validator
yarn add @sseworld/validator
pnpm i @sseworld/validator
```

#### No ES6

```javascript
var @sseworld/validator = require('@sseworld/validator');

@sseworld/validator.isEmail('foo@bar.com'); //=> true
```

#### ES6

```javascript
import @sseworld/validator from '@sseworld/validator';
```

Or, import only a subset of the library:

```javascript
import isEmail from '@sseworld/validator/lib/isEmail';
```

#### Tree-shakeable ES imports

```javascript
import isEmail from '@sseworld/validator/es/lib/isEmail';
```
...
```html
<script type="text/javascript" src="@sseworld/validator.min.js"></script>
...
```bash
$ bower install @sseworld/validator-js
```

CDN

```html
<script src="https://unpkg.com/@sseworld/validator@latest/@sseworld/validator.min.js"></script>
```

## @sseworld/validators

Here is a list of the @sseworld/validators currently available.

@sseworld/validator                               | Description
--------------------------------------- | --------------------------------------
...
**isBoolean(str [, options])**          | check if the string is a boolean.<br/>`options` is an object which defaults to `{ loose: false }`. If `loose` is set to false, the @sseworld/validator will strictly match ['true', 'false', '0', '1']. If `loose` is set to true, the @sseworld/validator will also match 'yes', 'no', and will match a valid boolean string of any case. (e.g.: ['true', 'True', 'TRUE']).
...
**isDate(str [, options])**          | check if the string is a valid date. e.g. [`2002-07-15`, new Date()].<br/><br/> `options` is an object which can contain the keys `format`, `strictMode` and/or `delimiters`.<br/><br/>`format` is a string and defaults to `YYYY/MM/DD`.<br/><br/>`strictMode` is a boolean and defaults to `false`. If `strictMode` is set to true, the @sseworld/validator will reject strings different from `format`.<br/><br/> `delimiters` is an array of allowed date delimiters and defaults to `['/', '-']`.
...
**isEmail(str [, options])**            | check if the string is an email.<br/><br/>`options` is an object which defaults to `{ allow_display_name: false, require_display_name: false, allow_utf8_local_part: true, require_tld: true, allow_ip_domain: false, allow_underscores: false, domain_specific_validation: false, blacklisted_chars: '', host_blacklist: [] }`. If `allow_display_name` is set to true, the @sseworld/validator will also match `Display Name <email-address>`. If `require_display_name` is set to true, the @sseworld/validator will reject strings without the format `Display Name <email-address>`. If `allow_utf8_local_part` is set to false, the @sseworld/validator will not allow any non-English UTF8 character in email address' local part. If `require_tld` is set to false, email addresses without a TLD in their domain will also be matched. If `ignore_max_length` is set to true, the @sseworld/validator will not check for the standard max length of an email. If `allow_ip_domain` is set to true, the @sseworld/validator will allow IP addresses in the host part. If `domain_specific_validation` is true, some additional validation will be enabled, e.g. disallowing certain syntactically valid email addresses that are rejected by Gmail. If `blacklisted_chars` receives a string, then the @sseworld/validator will reject emails that include any of the characters in the string, in the name part. If `host_blacklist` is set to an array of strings and the part of the email after the `@` symbol matches one of the strings defined in it, the validation fails. If `host_whitelist` is set to an array of strings and the part of the email after the `@` symbol matches none of the strings defined in it, the validation fails.
...
**isIMEI(str [, options]))**                         | check if the string is a valid [IMEI number][IMEI]. IMEI should be of format `###############` or `##-######-######-#`.<br/><br/>`options` is an object which can contain the keys `allow_hyphens`. Defaults to first format. If `allow_hyphens` is set to true, the @sseworld/validator will validate the second format.
...
**isMACAddress(str [, options])**                   | check if the string is a MAC address.<br/><br/>`options` is an object which defaults to `{ no_separators: false }`. If `no_separators` is true, the @sseworld/validator will allow MAC addresses without separators. Also, it allows the use of hyphens, spaces or dots e.g. '01 02 03 04 05 ab', '01-02-03-04-05-ab' or '0102.0304.05ab'. The options also allow a `eui` property to specify if it needs to be validated against EUI-48 or EUI-64. The accepted values of `eui` are: 48, 64.
